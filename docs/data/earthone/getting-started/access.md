---
title: Getting Access
description: How to create an account and get access to the EarthOne Platform
keywords:
  - EarthOne access
  - create account
  - platform access
  - EarthDaily Sales
  - service desk
  - getting started
---

# Getting access to the Platform

## Accessing the EarthOne Platform's petabytes of data and high performance computing resources

### Creating an Account

To create an account, please contact [EarthDaily Sales](https://earthdaily.com/contact).

Already have an account but don't have permission? File a ticket in our [Service Desk](http://support.descarteslabs.com).
