...
The EarthDaily® Platform offers polygon-based maps creation capabilities. You need to have in your layer tree at least one polygon layer loaded.
...
Description of the various map types is available [here](../Agro/Library/Map-types.md).

It is also possible to filter by the sensor used to acquire data, for example if user wants only Sentinel data. The following options are available:
...
Selecting one or more items on the coverage panel will casue them to appear in blue. The <Strong>"Back"</Strong> button allows you to come back to the Coverage Parameters options. The <Strong>"Next"</Strong> button allows you to go back to the map extraction panel.
